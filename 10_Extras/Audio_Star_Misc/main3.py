...
# --- Audio State ---
spectrum = np.zeros(60) # Array to hold frequency data
audio_data = None
sample_rate = 44100
is_playing = False
audio_mode = "MIC" 
...
def draw():
    ...
    # --- 1. Handle File Playback Frequency Calculation ---
    if audio_mode == "FILE" and is_playing:
        ...
        if idx < len(audio_data) - 2048:
            ...
            # Use log scaling to make the spectrum readable
            spectrum = np.log1p(np.abs(fft_data[:60])) * 5 
        else:
            is_playing = False
            audio_mode = "MIC"
    ...
    for i in range(num_bars):
        ...
        # Use the spectrum value directly for height
        h = min(spectrum[i] * 2, canvas.height)
...
def draw_ui():
    ...
    rect(20, 700, 100, 50); rect(130, 700, 100, 50); rect(240, 700, 100, 50) 
    fill(255); text_size(16)
    text("LOAD", 45, 730); text("PLAY", 160, 730); text("STOP", 270, 730)
    fill(255, 255, 0, 150)
